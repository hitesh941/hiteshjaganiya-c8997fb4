  ...
  return (
    <aside className="space-y-8 lg:sticky lg:top-28 self-start">
      ...
      <div className="bg-card rounded-2xl border border-border p-6 shadow-soft">
        ...
        <ul className="space-y-2">
          {categories.map(([category, count]) => (
            <li key={category}>
              <Link to="/blog" className="flex items-center justify-between text-sm text-muted-foreground hover:text-primary transition-colors">
                <span>{category}</span>
      ...
      <div className="bg-card rounded-2xl border border-border p-6 shadow-soft">
        ...
        <p className="text-sm text-muted-foreground mb-4">Have a question about growing your business? Drop me a message.</p>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <Textarea placeholder="Your message" rows={3} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} required />
