  ...
  return (
    <>
      ...
      <section className="section-padding bg-muted/30">
        <div className="container-custom"><div className="text-center max-w-3xl mx-auto">
          <span className="inline-block text-sm font-semibold text-primary uppercase tracking-wider mb-6">As Seen In</span>
          <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-10">Featured On</h3>
          <div className="flex flex-wrap justify-center items-center gap-6">
            <a href="https://theglobalhues.com" target="_blank" rel="noopener noreferrer" className="group p-5 bg-background rounded-2xl border border-border hover:border-primary/30 hover:shadow-medium transition-all duration-300"><img src={globalHuesLogo} alt="The Global Hues Magazine" className="h-10 md:h-12 w-auto object-contain group-hover:scale-105 transition-transform duration-300" /></a>
            <a href="https://yourstory.com" target="_blank" rel="noopener noreferrer" className="group p-5 bg-background rounded-2xl border border-border hover:border-primary/30 hover:shadow-medium transition-all duration-300"><img src={yourstoryLogo} alt="YourStory" className="h-10 md:h-12 w-auto object-contain group-hover:scale-105 transition-transform duration-300" /></a>
          </div>
        </div></div>
      </section>
      ...
      <section className="section-padding bg-background">
        <div className="container-custom"><div className="text-center max-w-3xl mx-auto">
          <span className="inline-block text-sm font-semibold text-primary uppercase tracking-wider mb-6">Professional Credentials</span>
          <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-10">Certifications</h3>
          <div className="flex flex-wrap justify-center items-center gap-4">
            {certifications.map((cert, index) => <div key={index} className="flex items-center gap-3 px-6 py-4 bg-muted/50 rounded-xl border border-border hover:border-primary/30 hover:shadow-soft transition-all duration-300"><span className="text-2xl">{cert.icon}</span><span className="font-medium text-foreground">{cert.name}</span><Award className="w-5 h-5 text-primary" /></div>)}
          </div>
        </div></div>
      </section>
