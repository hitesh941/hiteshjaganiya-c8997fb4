      ...
      <Helmet>
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            url: `${SITE_URL}/blog`,
            author: { "@type": "Person", name: "Hitesh Jaganiya", url: `${SITE_URL}/` },
          ...
          <div className="container-custom">
            <div className="max-w-4xl">
              ...
              <p className="text-lg md:text-xl text-muted-foreground max-w-3xl leading-relaxed">
                Things I've learned over 11 years of running SEO and Google Ads for businesses in Ahmedabad — written for founders, not for search engines.
              </p>
            </div>

            <div className="grid lg:grid-cols-[minmax(0,1fr)_280px] xl:grid-cols-[minmax(0,1fr)_300px] gap-8 xl:gap-12 mt-10 md:mt-14 items-start">
                  ...
                  <article key={post.slug} className="group bg-card rounded-2xl border border-border overflow-hidden shadow-soft hover:shadow-medium transition-all duration-300 hover:-translate-y-1">
                    <Link to={`/blog/${post.slug}`} className="block h-full">
                      <div className="overflow-hidden bg-secondary/30">
                        <img src={post.cover} alt={post.coverAlt} className="w-full aspect-[16/9] object-cover transition-transform duration-500 group-hover:scale-[1.02]" loading="lazy" />
                      </div>
                      <div className="p-5 md:p-6">
                        <div className="flex items-center gap-4 text-xs text-muted-foreground mb-3">
                          <span className="inline-flex items-center gap-1.5"><CalendarDays className="w-3.5 h-3.5" />{new Date(post.datePublished).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span>
                          <span className="inline-flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" />{post.readingTime}</span>
                        </div>
